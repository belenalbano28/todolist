 ...
 //console.log(response);
   ...
   array.forEach(element => {
    ...
    // 
     ...
     if(estado==0){
       x+='<div onclick="marcartarea('+idtarea+',1)" class="col-auto m-1 p-0 d-flex align-items-center"><h2 id="cambiar'+idtarea+'" class="m-0 p-0"><i id="checkear" class="fa fa-square-o text-primary btn m-0 p-0" data-toggle="tooltip" data-placement="bottom" title="Mark as complete"></i><i id="descheckear" class="fa fa-check-square-o text-primary btn m-0 p-0 d-none" data-toggle="tooltip" data-placement="bottom" title="Mark as todo"></i></h2></div>';
     }else{
       x+='<div onclick="marcartarea('+idtarea+',0)" class="col-auto m-1 p-0 d-flex align-items-center"><h2 id="cambiar'+idtarea+'" class="m-0 p-0"><i id="checkear" class="fa fa-square-o text-primary btn m-0 p-0 d-none" data-toggle="tooltip" data-placement="bottom" title="Mark as complete"></i><i id="descheckear" class="fa fa-check-square-o text-primary btn m-0 p-0" data-toggle="tooltip" data-placement="bottom" title="Mark as todo"></i></h2></div>';
     }
...
function marcartarea (id,estado){
  ...
  const data = { idtarea:id, estado:estado};
  $.ajax({
    type: 'POST',
    url: 'php/estadoTarea.php',
    data: data, 
  }).done(function (response) {
    if(response){
       traertareas();
